    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className={cn(
            "relative px-4 py-2 font-bold text-md transition-colors",
            hovered === idx
              ? "text-black"
              : "text-white dark:text-white hover:text-black dark:hover:text-black",
          )}
          key={`link-${idx}`}
          href={item.link}
        >
  ...
  return (
    <motion.div
      animate={{
        ...
        y: visible ? 20 : 0,
        backgroundColor: visible ? "#002d4b" : "transparent",
      }}
      transition={{
        type: "spring",
        stiffness: 200,
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 bg-[#002d4b] px-4 py-8 shadow-[0_0_24px_rgba(34,42,53,0.06),0_1px_1px_rgba(0,0,0,0.05),0_0_0_1px_rgba(34,42,53,0.04),0_0_4px_rgba(34,42,53,0.08),0_16px_68px_rgba(47,48,55,0.05),0_1px_0_rgba(255,255,255,0.1)_inset] dark:bg-[#002d4b]",
            className,
          )}
        >
          {children}
